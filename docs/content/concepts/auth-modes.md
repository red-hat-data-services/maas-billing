...
For the explicit priority order (API keys → OIDC → OpenShift TokenReview) and why OIDC must run before TokenReview, see [External OIDC Configuration](../advanced-administration/external-oidc.md#authentication-evaluation-order).
